...
## Tutorial 3: Analysis of Flex data

This tutorial demonstrates how to process 10x Chromium fixed RNA profiling (Flex) data using {{sc}}. The dataset consists of four mouse cell lines pooled into a single multiplexed Flex library.

This tutorial requires the `mouse_v1` probe set index. If it is not already listed in your `scprocess_setup.yaml`, add the following entry:
...
Then rerun {{scsetup}} to download and build the index:
...
scprocess newproj flex_project -s -c sc flex multiplex
...
```bash
cat data/metadata/flex_test_project_metadata.csv | column -s, -t
```
...
4plex_mouse_cell_lines_multiplex  A20_BC1       BC001
4plex_mouse_cell_lines_multiplex  3T3_BC2       BC002
4plex_mouse_cell_lines_multiplex  Neuro-2a_BC3  BC003
4plex_mouse_cell_lines_multiplex  EL4_BC4       BC004
```
...
The `newproj` command created a template at `config-flex_project.yaml`:
...
project:
  ...
  your_name: Testy McUser
  affiliation: Unemployed
  sample_metadata: data/metadata/flex_test_project_metadata.csv
  tenx_assay_type: flex
  probe_set: mouse_v1
  date_stamp: "2026-01-01"
multiplexing:
