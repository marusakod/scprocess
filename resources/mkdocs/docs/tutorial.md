...
Consider adding a `-n` or `--dry-run` flag to this command to verify the setup and get a list of all tasks {{sc}} will perform (see the [Usage](usage.md#4-run-the-analysis) section for more details).

??? note "{{sc}} dry run output for the tutorial dataset"

    If `--dry-run` flag is used with the {{scrun}} command you will see a detailed list of all steps that will be executed. This is the summary: 
...
All reports generated by {{sc}} for this tutorial are available at https://marusakod.github.io/scprocess_test_project/. 
...
All reports generated by {{sc}} for this tutorial are available at https://marusakod.github.io/scprocess_test_multiplexed_project/. 

## Tutorial 3: Processing 10x Flex data

This tutorial demonstrates how to process 10x Chromium Fixed RNA Profiling (Flex) data using {{sc}}. The dataset consists of four mouse cell lines pooled into a single multiplexed Flex library. To be able to follow this tutorial make sure you completed all the steps in the [Getting started](setup.md) section.

??? Info "More about the tutorial dataset"

    This tutorial uses the 10x Genomics public demonstration dataset "4-plex Mouse Cell Lines - Multiplex" (Cell Ranger 7.1.0). Four mouse cell lines (NIH/3T3, A20, EL4, and primary T cells) were profiled together in a single Flex library. Each cell line was labelled with a distinct probe barcode (BC001–BC004) prior to pooling.
...
```bash
# -c sc sets QC thresholds for single cell data
# flex sets tenx_assay_type: flex and probe_set instead of ref_txome
# multiplex adds the multiplexing section with demux_type: flex
scprocess newproj flex_project -c sc flex multiplex

cd flex_project
```

Next, download the raw FASTQ files:

!!! warning "File size"

    The FASTQ archive is approximately 99 GB. Download may take a while.
...
With `ls data/fastqs` you should be able to see FASTQ files following 10x Genomics naming conventions, for example:

```bash
4plex_mouse_cell_lines_multiplex_Multiplex_S1_L001_R1_001.fastq.gz
4plex_mouse_cell_lines_multiplex_Multiplex_S1_L001_R2_001.fastq.gz
4plex_mouse_cell_lines_multiplex_Multiplex_S1_L002_R1_001.fastq.gz
4plex_mouse_cell_lines_multiplex_Multiplex_S1_L002_R2_001.fastq.gz
...
```

Now create the sample metadata file at `data/metadata/flex_project_metadata.csv`. For Flex data the metadata file requires a `probe_id` column (instead of `hto_id`) mapping each sample to its probe barcode, and a `pool_id` column whose values match the FASTQ filename prefix:

```bash
cat data/metadata/flex_project_metadata.csv | column -s, -t
```

```bash
pool_id                                     sample_id   probe_id
4plex_mouse_cell_lines_multiplex_Multiplex  NIH3T3      BC001
4plex_mouse_cell_lines_multiplex_Multiplex  A20         BC002
4plex_mouse_cell_lines_multiplex_Multiplex  EL4         BC003
4plex_mouse_cell_lines_multiplex_Multiplex  primaryT    BC004
```

!!! info "Verifying probe barcode IDs"

    Probe barcode IDs (e.g. `BC001`) are assigned by 10x Genomics and are specific to the Flex kit version. You can confirm the correct IDs for your experiment from the kit documentation or from the Cell Ranger run summary (`web_summary.html`). For this dataset, the IDs and their corresponding cell lines can be verified from the [10x Genomics dataset page](https://www.10xgenomics.com/datasets).

### Creating a configuration file

The `newproj` command created a template at `config-flex_project.yaml`. Fill in the required fields, with the Flex-specific parameters highlighted below:

```yaml hl_lines="9 10 14"
project:
  proj_dir: /absolute/path/to/flex_project  # replace with correct absolute path
  fastq_dir: data/fastqs
  full_tag: flex_project
  short_tag: flex
  your_name: Testy McUser
  affiliation: Unemployed
  sample_metadata: data/metadata/flex_project_metadata.csv
  tenx_assay_type: flex
  probe_set: mouse_v1
  date_stamp: "2026-01-01"
multiplexing:
  demux_type: flex
...
Three parameters distinguish this configuration from a standard polyA workflow:

* `tenx_assay_type: flex` — switches the pipeline to Flex mode. {{sc}} will use `simpleaf multiplex-quant` instead of standard alignment, and will route outputs through `flex/` subdirectories
* `probe_set: mouse_v1` — specifies the probe set used to build the alevin-fry index. Use `mouse_v1` for the Chromium Fixed RNA Kit v1 (mouse); other valid values are `human_v1`, `mouse_v2`, and `human_v2`
* `demux_type: flex` — instructs {{sc}} to demultiplex samples from the pooled library using probe barcodes (from the `probe_id` column in the metadata)

!!! info "Probe set versions"

    The correct `probe_set` value must match the kit version used in your experiment. The kit version can be confirmed from the lot number printed on the reagent box or from the Cell Ranger run summary. As a rule of thumb: kits purchased before mid-2023 are typically v1; kits purchased after are typically v2.
...
```bash
scprocess run config-flex_project.yaml
```

The Flex mapping stage adds two steps before the standard ambient removal and QC steps:

1. **`run_mapping_flex`** — maps all reads from the pooled library to the probe-set index using `simpleaf multiplex-quant`, producing a single pool-level count matrix
2. **`save_alevin_flex_to_h5`** — runs once per sample; extracts that sample's barcodes from the pool by probe barcode prefix, writes a per-sample H5 file, and estimates the ambient RNA parameters

All downstream steps (ambient removal, QC, integration, marker genes, HTML reports) are identical to a standard polyA run.

??? note "{{sc}} dry run output for the Flex tutorial dataset"

    Running with the `--dry-run` flag shows the additional Flex mapping steps at the top of the execution plan:
    ...
    all                                          1
    run_mapping_flex                             1
    save_alevin_flex_to_h5                       4
    run_decontx                                  4
    run_qc_one_run                               4
    ...
    ```

    `run_mapping_flex` runs once for the single pool, while `save_alevin_flex_to_h5` runs once per sample (4 in this case).
