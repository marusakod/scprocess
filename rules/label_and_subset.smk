...
rule lbl_save_subset_sces:
  ...
  output:
    ...
    sce_ls      = expand(
      ...
      s = [] if not LBL_SCE_SUBSETS else [*LBL_SCE_SUBSETS]
    )
  threads: 4
  retries: RETRIES
  params:
    sub_names = ' '.join([] if not LBL_SCE_SUBSETS else [*LBL_SCE_SUBSETS])
  conda:
    '../envs/rlibs.yml'
  resources:
  ...
  shell:
    """
    if [ "{params.sub_names}" = "" ]; then
      echo "No subsets specified; skipping this rule."
    else
