...
run_fgsea <- function(mkrs_f, fgsea_go_bp_f, fgsea_go_cc_f, fgsea_go_mf_f,
  genome_ref, gsea_dir, min_cpm_go, max_zero_p, gsea_var, gsea_cut, not_ok_re, n_cores) {
  ...
  gsets_list  = get_fgsea_genesets(gsea_dir, genome_ref)
...
get_fgsea_genesets <- function(gsea_dir, genome_ref) {
  if (genome_ref %in% c("human_2024", "human_2020", "human_v1", "human_v2")) {
    gsets_list  = list(
  ...
  } else if (genome_ref %in% c("mouse_2024", "mouse_2020", "mouse_v1", "mouse_v2")) {
    gsets_list  = list(
    ...
    stop("unrecognised ref_txome / probe_set: '", genome_ref, "'")
  }
